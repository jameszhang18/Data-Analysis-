...
irwls_binomial_glm <- function(y,n,dat) {
  ...
  # X: dataframe containing the covariates as columns
...
# Compare with glm()
glm_coef <- coef(glm(cbind(dead,alive) ~ conc,data = bliss,family = binomial))
glm_coef
...
# doesn't look like it fits as weel as saturated980807
