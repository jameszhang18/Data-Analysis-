...
group_means <- pvc_t %>%
  group_by(operator) %>%
  summarize(group_mean = mean(psize),group_sd = sd(psize))
group_means
...
# Normality:
# 
# 2 random variable are equal in distribution 
# iff their cumulative distribution functions are equal
# Easier to compare thir INVERSE CDFs ~ the so called "quantile functions"
pvc_t %>%
...
# because observation in the tails are more inluential to sample statistics
# (ie. estimate of population qiantites)
#
# to get better undestanding: recreate the above but for data that is definitely not normally dstributed
# eg. rexp(100)
# or rpois(100)
# or even between rt(100,df=5)
...
# intercept is the mean of the function 
...
# need that to be true in order for the sum of squares decompostion as learned in class to hold
# need replication more than one datapoint for each combination
...
# so few data point -> not enough to conclude the variance is not consistent

# plot these plots on a grid, together we use the plot_grid function in the cowplow
...
data(warpbreaks)
warpbreaks_tbl <- as_data_frame(warpbreaks)
...
anova(warpbreaks_lm1,warpbreaks_lm2)
