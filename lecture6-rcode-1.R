...
# in saturated model, Rank == dimension, i.e "full rank" AND SQUARE, i.e. has no null-space.
...
# unrestricted: not assuming independence
# restricted: assuming independence => Pi*Pj
...
summary(hairpois1)
# y = beta_0 + (i-1) + (j-1) ge beta
# residual deviance compare with x^2_(i-1)*(j-1)=9
# poisson model with interaction => saturated model
# y = beta_0 + (i-1) + (j-1) + (i-1)(j-1) ge beta
...
haireye_with_mle %>%
  ...
  pull(lrt)
# log(unres/res) = full deviance - saturated deviance 
